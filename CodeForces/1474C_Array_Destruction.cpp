...
int main( int argc, char *argv[] )
{
    TestCase
    {
        int n,mx=0,ind,ans0=-1;
        sci(n);
        ind=2*n-1;
        int ara[2*n+9],f=0;
        map<int,int>mp,tmp;
        vector<pair<int,int>> ans;
        For(i,0,2*n-1) sci(ara[i]),mp[ara[i]]++;
        sort(ara,ara+2*n);
        ...
        For(i,0,2*n-2)
        {
            ...
            tmp[ara[2*n-1]]--;
            tmp[ara[i]]--;
            ans.pb({ara[2*n-1],ara[i]});
            if(n==1) f=1;
            For(j,0,n-2)
            {
                while(!tmp[ara[ind]]) ind--;
                tmp[ara[ind]]--;
                if(!tmp[mx-ara[ind]]) break;
                ans.pb({ara[ind],mx-ara[ind]});
                tmp[mx-ara[ind]]--;
                mx=max(ara[ind],mx-ara[ind]);
                if(j==n-2) f=1;
            }
            if(f==1) break;
        ...
        {
            printf("YES\n");
            pfi(ans[0].ff+ans[0].ss);pn;
            for(int i=0; i<ans.size(); i++)
            {
                pfi2(ans[i].ff, ans[i].ss);pn;
            }
        }
        else printf("NO\n");
