...
vector<pii> ans;
vector<int>spdiv(10000009);

void seive(int n)
{
    spdiv[1]=1;
    for(int i=2; i<=n; i++) spdiv[i]=i;
    for(int i=4; i<=n; i+=2) spdiv[i]=2;
    for(int i=3; i*i<=n; i++)
    {
        if(spdiv[i]==i)
        {
...
int main( int argc, char *argv[] )
{
    ...
    for(int i=0; i<n; i++)
    {
        ...
        int m;
            for(m=0; m<fac.size();m++)
            {
                //pfi(fac[m]),pfdot;pn;
                int o;
                for(o=m+1; o<fac.size();o++)
                {
                    if(__gcd(fac[m]+fac[o],ara[i])==1)
                    {
                        ans.pb({fac[m],fac[o]});
                        break;
                    }
...
//            {
//                int n;
//                for(n=m+1; n<fac.size();n++)
//                {
//                    if(__gcd(fac[m]+fac[n],ara[i])==1)
//                    {
//                        ans.pb({fac[m],fac[n]});
//                        break;
//                    }
