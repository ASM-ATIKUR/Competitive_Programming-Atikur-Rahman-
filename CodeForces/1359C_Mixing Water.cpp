...
double cal(double x, double c, double h)
{
    return (((x+1)*h)+(x*c))/(x*2+1);
}
...
int main( int argc, char *argv[] )
{
    TestCase
    {
        double h,c,t, avt=10e7, nt,nt1;
        scanf("%lf %lf %lf", &h,&c,&t);
        ll ans=LINF,l=0,r=LINF,mid;
        avt=(h+c)/2;
        if(avt>=t) pfi(2),pn;
        else
        {
