...
int main( int argc, char *argv[] )
{
    ...
    for(ll i=0; i<row; i++)
        cin>>s[i];
    ll dp[col+5][5] {0},wbcol[col+5][5] {0};
    ll ans=LINF,mn=INF,i,j,k;
    for(i=1; i<=col; i++)
    {
        ll cnt=0;
        for(j=0; j<row; j++)
        ...
        wbcol[i][0]=cnt;
        wbcol[i][1]=row-cnt;
        wbcol[i][0]+=wbcol[i-1][0];
        wbcol[i][1]+=wbcol[i-1][1];
    }
    for(i=1; i<=col; i++)
    {
        ll mn1=LINF, mn2=LINF,x1,x2,y1,y2;
        dp[i][1]=dp[i][0]=LINF;
        for(j=x; j<=y; j++)
        {
            if(i-j>=0)
            {
                ...
                y1=wbcol[i-j][0];
                y2=wbcol[i-j][1];
                dp[i][1]=min(x1+wbcol[i][1]-y2,dp[i][1]);
                dp[i][0]=min(x2+wbcol[i][0]-y1,dp[i][0]);
            }
            else x1=x2=y1=y2=0;
//            cout<<x1<<" "<<x2<<" "<<y1<<" "<<y2<<endl;
//            cout<<x1+wbcol[i][1]-y2<<" "<<x2+wbcol[i][0]-y1<<endl;

        }
//        cout<<dp[i][0]<<' '<<dp[i][1]<<endl;
    }
    cout<<min(dp[col][0],dp[col][1])<<endl;
