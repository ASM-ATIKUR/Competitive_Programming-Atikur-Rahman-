...
{
    int cnt[10] {0}, x,i,n,fg=1;
    sci(n);
    For(i,1,n) sci(x),cnt[x]++;

    if(cnt[1]!=n/3 || cnt[1]!=cnt[3]+cnt[2]) fg=0;
    if(cnt[3]>cnt[6]) fg=0;
    cnt[6]-=cnt[3];
    if(cnt[2]!=cnt[4]+cnt[6]) fg=0;
    if(fg)
    {
        For(i,1,cnt[3])
        {
        ...
        For(i,1,cnt[4]) printf("1 2 4\n");
        For(i,1,cnt[6]) printf("1 2 6\n");
    }
    else pfi(-1),pn;
