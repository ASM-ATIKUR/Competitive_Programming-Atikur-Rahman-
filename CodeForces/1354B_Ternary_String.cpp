...
int main( int argc, char *argv[] )
{
    ...
    TestCase
    {
        string s;
        cin>>s;
        int l=s.length(),p1=-1,p2=-1,i,ans=INF;
        vector<int> pre1(l+1),pre2(l+1),pre11(l+1),pre22(l+1);
        for(i=0; i<l; i++)
        {
            pre1[i]=p1;
            pre2[i]=p2;
        ...
        for(i=0; i<l; i++)
        {
            if(s[i]=='3')
            {
                if(pre1[i]!=-1 && pre2[i]!=-1) ans=min(ans,i-min(pre1[i],pre2[i])+1);
                if(pre1[i]!=-1 && pre22[i]!=-1) ans=min(ans, pre22[i]-pre1[i]+1);
                if(pre11[i]!=-1 && pre22[i]!=-1) ans=min(ans,max(pre11[i],pre22[i])-i+1);
                if(pre2[i]!=-1 && pre11[i]!=-1) ans=min(ans,pre11[i]-pre2[i]+1);
            }
        }
