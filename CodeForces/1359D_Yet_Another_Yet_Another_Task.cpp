...
int main( int argc, char *argv[] )
{
    int n;
    sci(n);
    vector<int>ara(n),lf(n),rt(n);
    trav(a,ara) sci(a);
    lf[0]=max(ara[0],0);
    rt[n-1]=max(ara[n-1],0);
    For(i,1,n-1)
    {
        lf[i]=max(lf[i-1]+ara[i],0);
    }
    rFor(i,n-2,0)
    {
        rt[i]=max(rt[i+1]+ara[i],0);
    }
    int ans=0;
    ans=min(rt[0]-ara[0],lf[n-1]-ara[n-1]);
    For(i,1,n-2)
    {
        cout<<lf[i]+rt[i]-ara[i]-ara[i]<<endl;
        ans=min(ans, lf[i]+rt[i]-ara[i]-ara[i]);
    }
    pfi(ans),pn;
