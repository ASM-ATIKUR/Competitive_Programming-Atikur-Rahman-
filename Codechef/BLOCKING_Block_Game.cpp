...
bool prefarence(int n, int h, int a, int b)
{
    //for(int i=0; i<n; i++)
    {
        return !(tim[a][h]>tim[b][h]);
    }
}

void Stable_Marrige(int n)
{
    int hfrnd[n+5], ans[n+5];
    bool ffree[n+5] {false};
    Ms(hfrnd, -1);
    int cnt=n;
    while(cnt>0)
    {
        int f;
        for(f=0; f<n; f++)
            if(ffree[f]==false) break;

        for(int i=0; i<n&&ffree[f]==false; i++)
        {
            if(hfrnd[i]==-1)
            {
                hfrnd[i]=f;
