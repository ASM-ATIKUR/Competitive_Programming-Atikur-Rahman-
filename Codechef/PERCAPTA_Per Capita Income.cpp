...
ll n,m;
vector<ll>graph[MX],income(MX),popu(MX),parrent(MX);
vector<bool>vis(MX);
pll mx;
...
//    P(u);pfdot;pfdot;pn;
    ...
    trav(x,graph[u])
    {
        if(!vis[x]&&(mx.ff*popu[x]==mx.ss*income[x]))
        {
            ret+=dfs(x,p);
        }
    }
...
int main( int argc, char *argv[] )
{
    TestCase
    {
        ...
        mx={0,1};
        scll2(n,m);
        for(i=1; i<=n; i++) scll(income[i]),vis[i]=false,parrent[i]=i;
        for(i=1; i<=n; i++)
        {
            scll(popu[i]);
//            if(i==1)
//                mx={income[i],popu[i]};
            if(mx.ff*popu[i]<mx.ss*income[i])
            {
                mx={income[i],popu[i]};
            }
            graph[i].clear();
...
//        P2(mx.ff,mx.ss);pn;
        for(i=1; i<=n; i++)
        {
            if(!vis[i]&&(mx.ff*popu[i]==mx.ss*income[i]))
            {
                ll tmp=dfs(i,i);
                if(tmp>ans)
                {
