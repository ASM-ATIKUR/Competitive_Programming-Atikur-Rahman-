...
int main( int argc, char *argv[] )
{
    TestCase
    {
        ...
        for(i=k+1; i<n+1; i++)
        {
            ...
            if(ara[i-k])
            {
                if(od.size()&&od.front()==dp[i-k])
                    od.pop_front();
            }
            else
            {
                if(ev.size()&&ev.front()==dp[i-k])
                    ev.pop_front();
            }
        }
        ans=INF;
        if(ev.size())
            ans=min(ans,ev.front()+1);
        if(od.size())
            ans=min(ans,od.front()+1);
        if(ans>=INF) pfi(-1);
        else pfi(ans);
        pn;
    }
