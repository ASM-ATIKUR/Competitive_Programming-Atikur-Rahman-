...
{
    ll n, x,ans=1,fg=1,zc=0;
    scll(n);
    while(n--)
    {
        scll(x);
        if(x==0) zc=1;
        else if(lg10(ans)+lg10(x)>18) fg=0;
        else if(lg10(ans)+lg10(x)==18)
        {
            ll a,b;
            if(lg10(ans)==0) a=ans;
            else a=ans/POW(10,lg10(ans));
            if(lg10(x)==0) b=x;
            else b=x/POW(10,lg10(x));
            if(a*b>1)
            {
                fg=0;
            }
    ...
    if(zc) pfi(0),pn;
    else if(fg && ans<=1000000000000000000) pfll(ans),pn;
    else pfi(-1),pn;
