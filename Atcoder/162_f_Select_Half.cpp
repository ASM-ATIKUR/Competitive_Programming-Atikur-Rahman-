...
ll frwrd[200100], bcwrd[200100],ara[200100];
...
//    freopen("output.txt", "w", stout);
    ll n,ans,i;
    scll(n);
    For(1,n+1) scll(ara[i]);
    frwrd[1]=ara[1];
    frwrd[2]=ara[2];
    for(i=3; i<=n; i++)
    {
        frwrd[i]=frwrd[i-2]+ara[i];
    }
    for(i=n; i>0; i--)
    {
        bcwrd[i]=bcwrd[i+2]+ara[i];
    }
    for(i=1; i<=n; i++)
    {
        printf("%lld  **  %lld\n", frwrd[i],bcwrd[i]);
    }
    ans=max(bcwrd[1],bcwrd[2]);
    pfll(ans),pn;
    if(n%2)
    {
        for(i=0; i<=n; i++)
