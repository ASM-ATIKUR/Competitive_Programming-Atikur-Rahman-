...
ll call(ll n, ll c, vector<pll> &data, ll nw, ll sum)
{
    if(nw==n)
        return 0;
    if(chk[nw][sum]!=-1) return chk[nw][sum];
    ll ret1,ret2=0;
    ret1=call(n,c,data,nw+1,sum);
    if(sum+data[nw].ff<=c)
        ret2=call(n,c,data,nw+1,sum+data[nw].ff)+data[nw].ss;
    return chk[nw][sum]=max(ret1,ret2);
}
