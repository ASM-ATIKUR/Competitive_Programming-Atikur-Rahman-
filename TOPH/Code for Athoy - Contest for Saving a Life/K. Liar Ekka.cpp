...
int main( int argc, char *argv[] )
{
    TestCase
    {
        string s,p;
        cin>>s>>p;
        int sl=s.length(),pl=p.length(),i,j,fg=0;
        vector<int> fr(sl+5),bc(sl+5);
...
//        }
        ...
        {
            //printf("%d...%d %d\n", i,fr[i],bc[i]);
            if(fr[i-1]+bc[i]>=pl) fg=1;
        }
        if(fg) printf("Ekka is not a Liar!\n");
        else printf("Ekka is a Liar!\n");
    }
