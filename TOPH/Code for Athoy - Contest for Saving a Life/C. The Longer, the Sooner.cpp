...
int main( int argc, char *argv[] )
{
    TestCase
    {
        int n;
        sci(n);
        vector<int>ara(n),chk(1001);
        for(int i=0; i<n; i++) sci(ara[i]);
        int ans=0;
        for(int i=0; i<n; i++)
        {
            if(ara[i]>0)chk[ara[i]]=max(chk[ara[i]],chk[ara[i]-1]+1);
            else if(ara[i]==0) chk[0]=1;
            ans=max(ans,chk[ara[i]]);
        }
        printf("Case #%d: ",cs++);pfi(ans),pn;
