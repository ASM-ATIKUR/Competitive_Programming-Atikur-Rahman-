...
ll n, dp[220][110], ara[220][110],cnt;
...
ll call(ll i, ll j)
{
    ...
    ll ret1=0,ret2=0;
    if(j==1) ret1=ara[i][j]+call(i+1, j);
    else if(j==ara[i][0]) ret2=ara[i][j]+call(i+1, j-1);
    else
    {
        ret1=ara[i][j]+call(i+1,j);
        ret2=ara[i][j]+call(i+1,j-1);
